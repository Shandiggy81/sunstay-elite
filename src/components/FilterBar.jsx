...
import { FILTER_CATEGORIES } from '../data/venues';
                ...
                {FILTER_CATEGORIES.map((filter, index) => {
                    ...
                    return (
                        <motion.button
                            ...
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            onClick={() => onFilterToggle(filter.tag)}
                            className={`px-4 py-2 rounded-full text-sm font-semibold transition-all whitespace-nowrap flex items-center gap-1.5 ${isActive
